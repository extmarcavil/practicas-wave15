...
public class Carrera {
    ...
    private SoccorristaAuto soccorristaAuto= new SoccorristaAuto();
    ...
    public void darDeAltaAuto(int velocidad, int aceleracion, int anguloGiro, String patente){
        if(listaDeVehiculos.size()<= cantidadDeVehiculosPermitidos){
            ...
            System.out.println("La carrera ya no puede agregar mas corredores");
        }
    }
    ...
    public void eliminarVehiculo(Vehiculo vehiculoAEliminar){
        if(listaDeVehiculos.contains(vehiculoAEliminar)){
            ...
            System.out.println("El vehiculo no existe en la lista");
        }
    }
    ...
    public Vehiculo vehiculoGanador(){
        ...
        for (Vehiculo v:listaDeVehiculos) {
            distaciaRecorriaPorAuto = (v.getVelocidad()*(1/2)*v.getAceleracion())/(v.getAnguloGiro()*(v.getPeso()-(v.getRuedas()*100)));
