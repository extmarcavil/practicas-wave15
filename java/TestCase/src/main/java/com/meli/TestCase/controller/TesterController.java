...
public class TesterController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ResPostTestDto> editarRegistroTest(@PathVariable Long id,
                                                             @RequestParam boolean test_passed){
        return new ResponseEntity<>(service.editar(id, test_passed), HttpStatus.OK);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<String> crearRegistroTest(@PathVariable Long id){
        return new ResponseEntity<>(service.eliminar(id), HttpStatus.OK);
    }

    @GetMapping("/fecha")
    public ResponseEntity<List<TestCase>> obtenerTodosPorFecha(@RequestParam String last_update){
        return new ResponseEntity<>(service.obtenerTodosPorFecha(last_update),HttpStatus.OK);
    }
}