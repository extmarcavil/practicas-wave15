...
public class PostService implements IPostService{
    ...
    /**
     * Crea un post y lo guarda en la lista de posts.
     * El método recibe un PostCreatePromoDTO, luego crea una variable local para obtener la fecha actual.
     * Luego se convierte el PostCreatePromoDTO en un objeto tipo Post, para guardar en la lista de posts.
     * @param newPost La información del post.
     * @return Retorna un booleano dependiendo de la respuesta del guardado.
     */
    @Override
    public boolean createPostPromo(PostCreatePromoDTO newPost) {
        var id = 0;
        var postlist = postRepository.all();
        id = postlist.size() == 0 ? 1 : postlist.size() + 1;
        var modelProduct = new Product();
        ...
        var date = zdt.toLocalDate();
        var modelPost = new Post(
                id, newPost.getUser_id(), date, Integer.valueOf(newPost.getCategory()), modelProduct,
                newPost.getPrice(), newPost.isHasPromo(), newPost.getDiscount());
    ...
    /**
     * This method count the posts of a seller
     * @param userId The id of the user
     * @return GetPostCountDTO is a DTO response to return
     */
    @Override
    public GetPostCountDTO getPostCountUser(int userId) {
        User user = userRepository.getUserById(userId);
        if (!user.isSeller())
            throw new NotSellerException("Error el usuario no es vendedor");

        List<Post> posts = postRepository.postPromosByUser(userId);
        GetPostCountDTO response = new GetPostCountDTO(user.getUserId(), user.getUserName(), posts.size());
        return response;
    }

    /**
     * This method return list of post with promo of a seller
     * @param userId The id of the user
     * @return GetPostPromoSellerDTO is a DTO response to return
     */
    @Override
    public GetPostPromoSellerDTO getListPostPromoSeller(int userId) {
        User user = userRepository.getUserById(userId);
        if (!user.isSeller())
            throw new NotSellerException("Error el usuario no es vendedor");

        GetPostPromoSellerDTO response = new GetPostPromoSellerDTO();
        List<PostPromoBySellerDTO> posts = new ArrayList<>();

        List<Post> sellerPosts = postRepository.postPromosByUser(userId);
        for (Post post: sellerPosts) {
                posts.add(new PostPromoBySellerDTO(post));
        }
        response.setPosts(posts);
