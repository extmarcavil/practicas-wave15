...
public class MeliRepository implements IMeliRepository {
    ...
    private static int contPublicationWithPromotion;
    static {
        users = new HashMap<>();
        ...
        cantPublicacion = 0;
        contPublicationWithPromotion = 0;
    }
    ...
    public List<Publication> getPromoPubications(Integer sellerId){

        return sellers.get(sellerId).getPublications()
                .stream().filter(p -> p.getHasPromo() != null && p.getHasPromo())
