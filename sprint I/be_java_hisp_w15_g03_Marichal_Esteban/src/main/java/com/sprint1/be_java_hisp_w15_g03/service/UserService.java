...
import com.sprint1.be_java_hisp_w15_g03.exception.*;
...
public class UserService implements IUserService {
    ...
    public UserService(IMeliRepository userRepository) {
        this.meliRepository = userRepository;
    ...
    public void followSeller(Integer userId, Integer sellerToFollow) {
        if (meliRepository.hasUser(userId) && meliRepository.hasSeller(sellerToFollow)) {
            if(!meliRepository.following(userId, sellerToFollow)){
                meliRepository.followSeller(userId, sellerToFollow);
            } else {
    ...
    public UserListDTO getFollowedList(Integer userId, String order) {
        if (meliRepository.hasUser(userId)) {
            User user = meliRepository.getUser(userId);
    ...
    public void unFollowSeller(Integer userId, Integer sellerToUnfollow) {
        if (meliRepository.hasUser(userId) && meliRepository.hasSeller(sellerToUnfollow)) {
            if(meliRepository.following(userId, sellerToUnfollow)){
                meliRepository.unFollowSeller(userId, sellerToUnfollow);
            } else {
