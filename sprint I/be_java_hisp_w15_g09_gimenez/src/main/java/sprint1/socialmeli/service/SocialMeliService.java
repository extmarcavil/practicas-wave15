...
public class SocialMeliService implements ISocialMeliService {
    ...
    @Override
    public List<TopUserDTO> getTop3Sellers () {
        List<TopUserDTO> listUsuario = new ArrayList<>();
        HashMap<Integer, User> hashMapUsuarios = repository.getAllUsers();
        hashMapUsuarios.forEach((k, v) -> listUsuario.add(new TopUserDTO(v.getId(), v.getListOfFollowers().size())));
        listUsuario.sort(Comparator.comparing(TopUserDTO::getFollowers).reversed());
        listUsuario.removeIf(person -> person.getFollowers() == 0);
        if (listUsuario.size() > 2) {
            if (listUsuario.size() > 3) {
                listUsuario.subList(3, listUsuario.size()).clear();
            }
        }
        return listUsuario;
    }
