...
public class UserService implements IUserService {
    ...
    @Override
    public PostNotUserIdDTO addPostPromo(NewPostPromoDTO newPostPromoDTO) {
        ...
        //postPromo.aplicarDescuento(); // si toca aplicar el descuento descomentar esta linea.
        user.addPostPromo(postPromo);
        return mapper.map(postPromo, PostNotUserIdDTO.class);
    }
    ...
    @Override
    public ProductsPromoCountDTO countPostPromo(Integer idUser) {
        User user = repository.findUser(idUser);
        ProductsPromoCountDTO productsPromoCountDTO = new ProductsPromoCountDTO(user.getUserID(), user.getName(), user.getPostPromos().size());
        return productsPromoCountDTO;
    }

    @Override
    public UserPromWhitListPromoDTO listOfProductsInProm(Integer userID){
        User user = repository.findUser(userID);
        List<PostNotUserIdDTO> postPromos = user.getPostPromos()
                .stream()
                .map(postPromo -> mapper.map(postPromo,PostNotUserIdDTO.class))
                .collect(toList());
        UserPromWhitListPromoDTO userPromWhitListPromoDTO = new UserPromWhitListPromoDTO(userID,user.getName(),postPromos);
        return userPromWhitListPromoDTO;
    }
