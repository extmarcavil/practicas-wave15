...
public class UserController {
    ...
    public ResponseEntity<PostPromoDTO> createPostPromo(@RequestBody NewPostPromoDTO newPostPromoDTO) {
        PostPromoDTO postPromoDTO1 = service.addPostPromo(newPostPromoDTO);
        return new ResponseEntity<>(postPromoDTO1, HttpStatus.ACCEPTED);
    }

    @GetMapping("/products/promo-post/list")
    public ResponseEntity<?> getPromoList(@RequestParam Integer user_id){
        PostPromoListDTO postPromoListDTO = service.getPromoList(user_id);
        return new ResponseEntity<>(postPromoListDTO, HttpStatus.ACCEPTED);
    }

    @GetMapping("/products/promo-post/count")
    public ResponseEntity<?> promoCount(@RequestParam Integer user_id){
        PostPromoCountDTO postPromoCountDTO = service.promoCount(user_id);
        return new ResponseEntity<>(postPromoCountDTO, HttpStatus.ACCEPTED);
    }
}
