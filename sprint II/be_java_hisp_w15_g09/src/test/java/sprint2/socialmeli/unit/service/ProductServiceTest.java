...
public class ProductServiceTest {
    ...
    public void verifyPostLastTwoWeeks () {
        ...
        Mockito.when(IsocialMeliRepository.findUserById(1)).thenReturn(nico);
        Mockito.when(postRepository.getListOfPostOfUser(2)).thenReturn(listaPost);
    ...
    private int mockFindUserByID(User aMockUser) {
        ...
        Mockito
                .when(
                        this.IsocialMeliRepository.findUserById(testId))
                .thenReturn(
                        aMockUser
                );
        return testId;
