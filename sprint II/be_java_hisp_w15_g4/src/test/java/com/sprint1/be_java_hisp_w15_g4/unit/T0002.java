...
public class T0002 {
    ...
    @Test
    @DisplayName("No puede dejar de seguir a quién no sigue")
    void unfollowThrowsException(){
        //Arrange
        int userId = 1;
        int userIdTwo = 2;
        User user = new User(userId, "nombre1");
        User userTwo = new User(userIdTwo, "nombre2");
        // Mock
        Mockito.when(repo.findUser(userId)).thenReturn(user);
        Mockito.when(repo.findUser(userIdTwo)).thenReturn(userTwo);

        //Act & Assert
        Assertions.assertThrows(NotFollowException.class, () -> service.unfollow(user.getUser_id(), 2));
    }
}
