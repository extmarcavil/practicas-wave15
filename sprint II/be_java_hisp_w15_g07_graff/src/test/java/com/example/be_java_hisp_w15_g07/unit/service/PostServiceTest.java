...
public class PostServiceTest {
    ...
    public void addTestNewPost(){
        ...
        Product newProdRepo = new Product();
        NewPostDTO newPost = new NewPostDTO(userX.getUserId(), LocalDate.now(), newProd, 1, 120.00 );
        Post newPostRepo = new Post(LocalDate.now(), newProdRepo, 1, 120.00);
        //mock
        //when(repository.findById(userX.getUserId())).thenReturn(userX);

        doNothing().when(repository).newPost(any(Integer.class), any(Post.class));
        //act
        service.newPost(newPost);
        //assert
        verify(repository, times(1)).newPost(any(Integer.class), any(Post.class));
    }
}
