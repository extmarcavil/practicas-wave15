...
public class Controller {
    ...
    public String convert(@PathVariable String code) {
            ...
            String[] arregloDePalabras = code.split("   ");
            for (String array : arregloDePalabras) {
                String[] arregloDeLetras = array.split(" ");
                for (int i = 0; i < arregloDeLetras.length; i++) {
                    String letra = mapMorse().get(arregloDeLetras[i]);
                    result.append(letra);
                }
                result.append(" ");
            }
